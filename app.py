...
# --- 2. OFERTA ACADÉMICA CALIFICADA (CONSOLIDADA TOTAL) ---
...
saved = cookies.get("fce_econ_v_master_v1")
if saved:
...
with st.sidebar:
    ...
    if st.button("💾 GUARDAR DATOS"):
        ...
        cookie_manager.set("fce_econ_v_master_v1", json.dumps(data))
        st.success("Guardado.")
        ...
        if validos:
            ...
            for i, combo in enumerate(validos[:2]):
                ...
                for idx, c in enumerate(combo):
                    ...
                    # Lógica de probabilidad basada en Ranking y Registro
                    if u_rank > c[6] + 10: prob, color = "ALTA", "#059669"
                    elif u_rank > c[6] - 10:
                        if int(u_reg) <= c[7]: prob, color = "MEDIA (Por Registro)", "#D97706"
                        else: prob, color = "BAJA (Ranking Límite)", "#DC2626"
