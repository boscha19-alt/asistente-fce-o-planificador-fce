...
PLAN_ECON = {
    ...
    "Ciclo Profesional": {
        ...
        555: ["Org. Industrial", [286]], # Correlativa: Micro II
    ...
    "Optativas": {
        520: ["Ciencia Datos", [543]], # Correlativa: Econometría I
        521: ["Econ. Austriaca", [242]], 
        763: ["Teoría Juegos", [291]],
        563: ["Innovación", [242]]
    }
}

# --- 2. OFERTA ACADÉMICA MAESTRA (PROCESADA DEL PDF COMPLETO) ---
# [Cod, Cátedra, Profesor, Días, Horario, Sede, Ranking_Corte, Modalidad, Día_Virtual]
OFERTA_TOTAL = [
    # --- Finanzas Públicas (556) - AGREGADO ---
    [556, "CURCIO", "Curcio Javier", "Ma/Vi", "17-19", "Córdoba", 175.7, "P", ""],
    [556, "CURCIO", "Costa Augusto", "Lu/Ju", "09-11", "Córdoba", 170.0, "P", ""],
    [556, "CURCIO", "Goldschmit Ariela", "Ma/Vi", "09-11", "Córdoba", 172.0, "P", ""],
    [556, "CURCIO", "Borenstein Andres", "Lu/Ju", "17-19", "Córdoba", 168.5, "P", ""],
    [556, "CURCIO", "Rossignolo Dario", "Ma/Vi", "19-21", "Córdoba", 165.0, "P", ""],
    
    # --- Ciencia de Datos (520) - AGREGADO ---
    [520, "DPTO. ECONOMÍA", "Sidicaro Nicolas", "Ma/Vi", "09-11", "Córdoba", 183.9, "P", ""],
    [520, "DPTO. ECONOMÍA", "Semeshenko / De Raco", "Ma/Vi", "11-13", "Córdoba", 180.0, "P", ""],
    ...
    [555, "PETRECOLLA", "Romero G. E.", "Lu/Ju", "17-19", "Córdoba", 197.1, "P", ""],
...
saved = cookies.get("fce_econ_v_final_full_all_v2")
if saved:
...
with st.sidebar:
    ...
    with st.expander("2. Ciclo Profesional", expanded=cbc_ok):
        for cod, info in PLAN_ECON["Ciclo Profesional"].items():
            # Fix lógica 555 y demás: chequeo recursivo de correlativas
            ...
            if bloq: st.caption(f"🔒 Bloqueada. Falta: {faltan if cbc_ok else 'CBC'}")
    ...
    if st.button("💾 GUARDAR DATOS"):
        ...
        cookie_manager.set("fce_econ_v_final_full_all_v2", json.dumps(data))
        st.success("Guardado.")
